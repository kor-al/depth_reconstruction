...
def SRGAN_g(d_input,t_interpolated, is_train=True, reuse=False):
    ...
    with tf.variable_scope('G_Depth_SR', reuse=reuse):
        net['conv1'] = residual_block(net['gate'], base_filters * 2, 'convolution_1', kernel_sz=3, act=tf.nn.elu, bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['conv2'] = residual_block(net['conv1'], base_filters * 2, 'convolution_2', kernel_sz=3, act=tf.nn.elu, bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['drop1'] = drop(net['conv2'], p, is_training=is_train)

        net['conv3'] = residual_block(net['drop1'], base_filters * 2, 'convolution_3', kernel_sz=3, act=tf.nn.elu, bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['conv4'] = residual_block(net['conv3'], base_filters * 2, 'convolution_4', kernel_sz=3, act=tf.nn.elu, bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['drop2'] = drop(net['conv4'], p, is_training=is_train)

        net['conv5'] = residual_block(net['drop2'], base_filters * 2, 'convolution_5', kernel_sz=3, act=tf.nn.elu, bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['conv6'] = residual_block(net['conv5'], base_filters * 2, 'convolution_6', kernel_sz=3, act=tf.nn.elu, bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['drop3'] = drop(net['conv6'], p, is_training=is_train)

    
        net['conv7'] = conv2d(net['drop3'], base_filters * 4, 'upconv_1', stride=1, kernel_sz=3, act=tf.nn.elu, w_init=w_init)
        net['uconv1'] = pixelShuffler(net['conv7'], scale=2)
        #net['uconv1'] = tf.depth_to_space(net['conv7'], 2)

        net['conv8'] = conv2d(net['uconv1'], base_filters * 4, 'upconv_2', stride=1, kernel_sz=3, act=tf.nn.elu,w_init=w_init)
        net['uconv2'] = pixelShuffler(net['conv8'], scale=2)
        #net['uconv2'] = tf.depth_to_space(net['conv8'], 2)

        net['conv9'] = conv2d(net['uconv2'], base_filters * 4, 'upconv_3', stride=1, kernel_sz=3, act=tf.nn.elu, w_init=w_init)
...
def SRGAN_d(input_images, is_train=True, reuse=False):
    ...
    with tf.variable_scope("SRGAN_d", reuse=reuse):
        ...
        net['conv1'] = conv2d(net['gate'], df_dim,kernel_sz = 3, stride = 2, act=lrelu, name='convolution_1', bn =False, w_init=w_init)
        net['drop1'] = drop(net['conv1'], p, is_training=is_train)
        net['conv2'] = conv2d(net['drop1'], df_dim * 2,kernel_sz = 3, stride = 1, act=lrelu, name='convolution_2', bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['drop2'] = drop(net['conv2'], p, is_training=is_train)
        net['conv3'] = conv2d(net['drop2'], df_dim * 4,kernel_sz = 3, stride = 2, act=lrelu, name='convolution_3', bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['drop3'] = drop(net['conv3'], p, is_training=is_train)
        net['conv4'] = conv2d(net['drop3'], df_dim * 8,kernel_sz = 3, stride = 2, act=lrelu, name='convolution_4',bn = True, is_training=is_train, w_init=w_init, g_init=g_init)
        net['drop4'] = drop(net['conv4'], p, is_training=is_train)
        net['conv5'] = conv2d(net['drop4'], df_dim * 2,kernel_sz = 3, stride = 1, act=lrelu, name='convolution_5', bn = True, is_training=is_train, w_init=w_init, g_init=g_init)

        net['flatten'] = l.flatten(net['conv5'],scope = 'flatten')
        net['out'] = l.fully_connected(net['flatten'], 1, activation_fn=None, scope='dense', weights_initializer = w_init)
