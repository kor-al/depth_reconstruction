...
n_epoch_init_d = config.TRAIN.n_epoch_init_d
...
def train():
    ...
    g_init_loss =  mse_loss +vgg_loss+ tv_loss
    g_loss = vgg_loss + g_gan_loss + mse_loss + tv_loss
    ...
    with tf.variable_scope('learning_rate'):
        ...
        ## Pretrain
        g_optim_init = tf.train.AdamOptimizer(lr_v, beta1=beta1).minimize(g_init_loss, var_list=g_vars, global_step=glob_step_t)
        ## SRGAN
    ...
    for epoch in range(0, n_epoch_init + 1):
        try:
            ...
            batch_it = tqdm(SynthiaIterator(valid_hr_img_list, batchsize=batch_size, shuffle=True, buffer_size=70),total=n_batches_valid, leave=False)
            for b in batch_it:
                xb = b[0]
            ...
            print("Validation: Epoch {0} val mae {1} val mse {2}".format(epoch - 1, val_mae/n_batches_valid, val_mse/n_batches_valid))

            total_mse_loss, total_g_loss = 0, 0
            batch_it = tqdm(SynthiaIterator(train_hr_img_list, batchsize=batch_size, shuffle=True, buffer_size=70), total=n_batches, leave=False)
            for b in batch_it:
                xb = b[0]
                xb = augment_imgs(xb)
                glob_step, errM, errG, _ = sess.run([glob_step_t,mse_loss,g_loss, g_optim_init], feed_dict={t_input: xb, d_flg:True})
            ...
            log = "[*] Epoch: [%2d/%2d] time: %4.4fs, mse: %.8f" % (epoch, n_epoch_init, time.time() - epoch_time, total_mse_loss / n_batches)
            ...
            train_mse_loss_summary = tf.Summary.Value(tag='g_init/train_mse_loss', simple_value=total_mse_loss / n_batches)
            ...
            print(log)
            saver_g.save(sess, os.path.join(checkpoint_dir + '/g_init','model' + str(epoch)+'.ckpt'))
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        try:
            ...
            for b in batch_it:
                xb = b[0]
                errM,mae_score, errG, errD = sess.run([mse_loss, mae, g_loss, d_loss],feed_dict={t_input: xb, d_flg:False})
                val_mae+=mae_score
                val_mse+=errM
                val_g_loss+=errG
                val_d_loss+=errD
            ...
            print("Validation (GAN): Epoch {0} val mae {1} val mse {2}".format(epoch - 1, val_mae/n_batches_valid, val_mse/n_batches_valid))
            ...
            for b in batch_it:
                ...
                ## update G
                glob_step, errG, errM, _, summary = sess.run([glob_step_t, g_loss, mse_loss, g_optim, summary_op], {t_input: xb, d_flg:True})
                total_mse_loss +=errM
                total_d_loss += errD
                total_g_loss += errG
        ...
        epoch, n_epoch, time.time() - epoch_time, total_d_loss / n_batches, total_g_loss / n_batches, total_mse_loss/n_batches)
        ...
        epoch_summary = tf.Summary(value=[val_mse_summary,val_g_loss_summary,val_d_loss_summary, train_mse_loss_summary, train_g_loss_summary, train_d_loss_summary])
