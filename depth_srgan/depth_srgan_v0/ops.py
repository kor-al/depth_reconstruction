...
def conv2d(in_tensor, out_dim, name='conv', kernel_sz=3, act=None, stride=1, bn=False, is_training = True, w_init = None, g_init = None):
    with tf.variable_scope(name):
        ...
        out = l.conv2d(in_tensor,
                       ...
                       activation_fn=l_act,
                       padding='SAME',
                       normalizer_params={'activation_fn': act, 'is_training': is_training,'param_initializers':{'gamma':g_init}})
    return out


def projection_block(in_tensor, out_dim, name='conv', kernel_sz=3, act=None, stride=1, bn=False, is_training = True, w_init = None, g_init = None):
    with tf.variable_scope(name):
        if bn:
            norm = l.batch_norm
            l_act = None
...
def residual_block(in_tensor, out_dim, name='conv', kernel_sz=3, act=None, stride=1, bn=False, is_training=True, w_init = None, g_init = None):
    with tf.variable_scope(name):
        if bn:
            norm = l.batch_norm
            l_act = None
