...
class SRGBDIterator(AsyncBatchIterator):
    ...
    def _read(self, files):
        from utils.utils import open_depth_synthia, open_rgb, open_depth
...
class SynthiaIterator(AsyncBatchIterator):
    ...
    def _read(self, files):
        from utils.utils import open_depth_synthia, open_rgb
        if any(['Depth' in f for f in files]):
...
class MPIterator(AsyncBatchIterator):
    def __init__(self, *args, **kwargs):
        super(MPIterator, self).__init__(*args, **kwargs)

    def _read(self, files):
        from utils.utils import open_depth_mp, open_rgb
        if any(['color' in f for f in files]):
            
            images = np.asarray([open_rgb(f) for f in files])
        else:
            #returns depth
            images = np.asarray([open_depth_mp(f)  for f in files])
        return images
...
class FreiburgIterator(AsyncBatchIterator):
    ...
    def _read(self, files):
        from utils.utils import open_rgb, open_depth_freiburg
...
class TUMIterator(AsyncBatchIterator):
    ...
    def _read(self, files):
        from utils.utils import open_depth_tum, open_rgb
        if any(['depth' in f for f in files]):
            #returns depth in meters
